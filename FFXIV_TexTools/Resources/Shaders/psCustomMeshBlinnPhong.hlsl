...
float4 calcBlinnPhongLighting(float4 LColor, float4 vMaterialTexture, float3 N, float4 diffuse, float3 L, float3 H, float4 specular, float shininess, float3 r)
{
    //float4 Id = vMaterialTexture * diffuse * saturate(dot(N, L));
    //float4 Is = specular * pow(saturate(dot(r, H)), sMaterialShininess);
    float4 f = lit(dot(N, L), dot(r, H), shininess);
...
{    
    ...
    for (int i = 0; i < NumLights; ++i)
    {
        if (Lights[i].iLightType == 1) // directional
        {
            ...
            DI += calcBlinnPhongLighting(Lights[i].vLightColor, vMaterialTexture, input.n, input.cDiffuse, d, h, specular, shininess, r);
        }
        else if (Lights[i].iLightType == 2)  // point
        {
			...
			float3 r = reflect(-d, input.n);
            DI = mad(att, calcBlinnPhongLighting(Lights[i].vLightColor, vMaterialTexture, input.n, input.cDiffuse, d, h, specular, shininess, r), DI);
        }
